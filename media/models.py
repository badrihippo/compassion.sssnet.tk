...
class Picture(models.Model):
    owner = models.ForeignKey('auth.User', related_name='my_pics', null=True)
    caption = models.CharField(max_length=300,blank=True)
    description = models.TextField(blank=True)
    image = models.ImageField(upload_to='img/photos', blank=True, null=True)
    ...
    def __unicode__(self):
        if self.caption:
            ...
            title = self.description[:20]
        return '%(title)s by %(person)s' % {
            'person': self.owner,
            'title': title
            }
...
# Country, state and address models thanks to MasonM on Django Snippets:
